										...
										<div class="d-md-flex post-entry-2 half">
											<div style="background-color: white; border: 2px solid white; width: 20%; border-radius: 10px;">
												<div class="mom-select-img">
													<div>
														<img alt="" src="resources/img/${c.mm.user_pic }">
													</div>
												</div>
											</div>
											<div style="padding-left: 25px;">
												...
												<div class="mom-select-locations">
													...
													<div class="mom-select-location">
														<div>${c.cs_location1 }</div>
														<div>${c.cs_location2 }</div>
														<div>${c.cs_location3 }</div>
													</div>
												</div>
											</div>
										</div>
										...
										<div class="col text-center">
											<div class="text-start py-4"
												...
												<div class="custom-pagination">
													<c:if test="${curPage != 1 }">
														<a href="page.change.caresitter?p=${curPage - 1}&cs_search=${searchSession.cs_search}" class="prev">Previous</a>
													</c:if>
													<c:forEach begin="${startPage}" end="${endPage}" varStatus="loop">
														<c:choose>
															<c:when test="${curPage == loop.index}">
																<a
																	href="page.change.caresitter?p=${loop.index}&cs_search=${searchSession.cs_search}"
																	class="active">${loop.index}</a>
															</c:when>
															<c:otherwise>
																<a
																	href="page.change.caresitter?p=${loop.index}&cs_search=${searchSession.cs_search}">${loop.index}</a>
															</c:otherwise>
														</c:choose>
													</c:forEach>
													<c:if test="${curPage != pageCount }">
														<a
															href="page.change.caresitter?p=${curPage + 1}&cs_search=${searchSession.cs_search}"
															class="prev">Next</a>
													</c:if>
												</div>
											</div>
